let wsServer = require('ws').Server, uid = 'chufan.lai.1990@gmail.com'
let testClient = require('../vast2016MC3/testClient').initialize(), stream, timer, t_mins = 1, choiceTimer
let t_send = function (v_ws, v_message) {
  if (v_ws.OPEN) {
    let t_result = v_ws.send(v_message)
    if (t_result == 'error') {
...
function initialize (v_server, v_db, logger, v_test) {
  let streamServer = new wsServer({server: v_server}), messageHandler, Timeleft = 60
  ...
  streamServer.on('connection', function connection (v_ws) {
    ...
    v_ws.on('message', function incoming (message) {
      let t_message = JSON.parse(message), self = this
  ...
  streamServer.queryHVAC = function (v_ws) {
    let t_ready = [false, false, false, false, false], t_data = [], t_timer, tt_data,
